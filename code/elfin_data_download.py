...
def download_all_elfin_data(delete_prior = False):
    ...
    if delete_prior == True:
        ...
        os.system("rm -f ./data/raw/*");                       print("rm -f ./data/raw/*")
        os.system("rm -f ./data/processed_scientific_data/*"); print("rm -f ./data/processed_scientific_data/*")
        os.system("rm -f ./data/processed_position_data/*");   print("rm -f ./data/processed_position_data/*\n")
...
# Checks data availability for a date, downloads it if available, and cleans it.
    ...
    raw_path = f"{os.getcwd()}/data/raw/science_{date}_el{sat}.cdf" # Path to save the raw .cdf to
    processed_path = f"{os.getcwd()}/data/processed_scientific_data/{date}_el{sat}.npz" # Path to save the cleaned .npz to
...
def _get_position_data(date, sat):
    ...
    raw_path = f"{os.getcwd()}/data/raw/{url[-29:]}" # Path to save the raw .cdf to
    processed_path = f"{os.getcwd()}/data/processed_position_data/{date}_el{sat}.npz" # Path to save the cleaned .npz to
...
def download_metadata():
    for sat in ["a", "b"]:
        ...
        file = open(f"{os.getcwd()}/data/el{sat}_epd_all.csv", 'wb')
    ...
    for i in range(len(geomag_indices_urls)):
        ...
        file = open(f"{os.getcwd()}/data/{name}.csv", 'wb')
        file.write(r.content)
        file.close()
