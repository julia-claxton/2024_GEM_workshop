...
This script provides the type Event used for storing and representing ELFIN data.
...
# Creates an Event based on the name string of another event
    ...
    end_datetime   = DateTime(date, time + duration)

    return create_event(start_datetime, end_datetime, sat, warn = warn)
end
...
# Creates an Event object from a date and satellite name. Event covers all data recorded on the given date.
    ...
    end_datetime   = DateTime(date, Time("23:59:59"))

    return create_event(start_datetime, end_datetime, sat, warn = warn)
end

function create_event(start_datetime::DateTime, end_datetime::DateTime, sat; warn = false)
# Creates an Event object from a start datetime, stop datetime, and satellite ID (either "a", "A", "b", or "B")
    ...
    science_data_path = "$(dirname(dirname(@__DIR__)))/data/processed_scientific_data/$(year)$(month)$(day)_el$(sat).npz"
    position_data_path = "$(dirname(dirname(@__DIR__)))/data/processed_position_data/$(year)$(month)$(day)_el$(sat).npz"
    ...
    data = load_data(science_data_path, position_data_path)

    # Check bounds
    time_from_start = data["fs_time"] .- start_datetime
    time_from_end   = data["fs_time"] .- end_datetime
    ...
    if min(abs.(time_from_end)...) >= Second(2) # If the nearest time is more than 2 seconds away. This means ~10 missed observations at ELFIN's sampling about every .2 seconds.
        ...
        old_end = end_datetime
        end_datetime = data["fs_time"][idx] # Set start to nearest time we have data for.
    ...
    indices_of_interest = (data["fs_time"] .>= start_datetime) .& (data["fs_time"] .<= end_datetime)
    ...
    if n_datapoints == 0
        @warn "\033[93mNo data exists between start time and end time (Δt = $(end_datetime - start_datetime)), no event created.\033[0m"
        return nothing
    end
    ...
    dst, kp = get_geomagnetic_indices(time_datetime)
    ...
    if n_datapoints == 0
        ...
        end_time   = Dates.format(end_datetime, dateformat"HH:MM:SS")
    ...
    binned = bin_data(data, n_datapoints, indices_of_interest)
    if binned == nothing
        # Warning contained in bin_data()
        return nothing
    end

    # Calculate loss/anti-loss cones over the observation
    calculate_loss_cones(data, binned, n_datapoints, indices_of_interest) # Mutates data dict
    calculate_Jprec_over_Jperp(data, binned)
...
function load_data(science_data_path, position_data_path)
# Load ELFIN data from cleaned .npz into a dictionary
    ...
    return Dict("satellite"        => satellite,
                "et_time"          => timestamp_to_DateTime(get(science_data, :et_time)),
                "hs_time"          => timestamp_to_DateTime(get(science_data, :hs_time)),
                "fs_time"          => timestamp_to_DateTime(get(science_data, :fs_time)),
                ...
                "state_time"       => timestamp_to_DateTime(get(position_data, :state_time)),
...
function timestamp_to_DateTime(times)
# Convert timestamp string to DateTime object
    date_time = String.(collect(times))
    date_time = DateTime.(date_time, dateformat"yyyy.mm.dd.HH.MM.SS.sss")
end

function get_geomagnetic_indices(time_datetime)
    results = Dict()
    for index_name in ["dst", "kp"]
        path = "$(dirname(dirname(@__DIR__)))/data/$(index_name).csv"
...
function bin_data(data, n_datapoints, indices_of_interest)
# Bin pitch angle and flux data into ELFIN's discrete look directions
    ...
    if sum(isnan.(nflux)) + sum(isnan.(eflux)) + sum(isnan.(pitch_angles)) ≠ 0
        if warnings == true; @warn "\033[93mbin_data(): Data contains NaN. Aborting event creation.\033[0m"; end
        return nothing
    end
...
function calculate_loss_cones(data, binned, n_datapoints, indices_of_interest)
...
function calculate_Jprec_over_Jperp(data, binned)
    time = data["time"]
...
function integrate_flux(event::Event; time = false, pitch_angle = false, energy = false, 
    ...
    if pitch_angle == true
       e_flux, n_flux = _integrate_over_pitch_angle(event, e_flux, n_flux, pitch_angle_idxs, correct_lc)
       pa = 1
    end

    if energy == true
        e_flux, n_flux = _integrate_over_energy(event::Event, e_flux, n_flux, energy_range_idx)
        e = 1
    end

    if time == true
        e_flux, n_flux = _integrate_over_time(event::Event, e_flux, n_flux, time_range)
        t = 1
    end
...
function _integrate_over_pitch_angle(event::Event, e_flux, n_flux, idxs, correct_lc)
    for t = 1:event.n_datapoints
        ...
        pa = event.pitch_angles[t, idxs[t]]

        # Get percent of loss cone we are covering so we can correct for coverage in loss cone
        correction_factor = 1 # Normal behaviour

        lc_angle = event.loss_cone_angles[t]
        if correct_lc && (lc_angle < 90) # Northern hemisphere
            lc_size_deg = lc_angle
            max_distance_into_lc = lc_angle - min(pa...)
            lc_coverage = max_distance_into_lc / lc_size_deg
            correction_factor = 1/lc_coverage # This might cause problems by blowing up if we just barely clip the loss cone

        elseif correct_lc && (lc_angle > 90) # Southern hemisphere
            lc_size_deg = 180 - lc_angle
            max_distance_into_lc = max(pa...) - lc_angle
            lc_coverage = max_distance_into_lc / lc_size_deg
            correction_factor = 1/lc_coverage # This might cause problems by blowing up if we just barely clip the loss cone
        end
        # If we have less than 10% coverage of the loss cone, don't extrapolate that data to the whole thing.
        if correction_factor > 10
            correction_factor = 0
        end

        for E = 1:16
            e_flux[t, E, :] .= 2 * π * integrate(pa, e_flux[t, E, idxs[t]] .* sind.(pa), Trapezoidal()) * correction_factor
            n_flux[t, E, :] .= 2 * π * integrate(pa, n_flux[t, E, idxs[t]] .* sind.(pa), Trapezoidal()) * correction_factor
        end
    end
...
function _integrate_over_time(event::Event, e_flux, n_flux, time_range)
    if event.n_datapoints == 1
        return event.e_flux[:, :, :] .* 3,  event.n_flux[:, :, :] .* 3 # T_spin ~= 3 seconds
    end
